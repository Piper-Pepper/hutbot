...
async def sync_model_caps_from_api(session: aiohttp.ClientSession):
    if not VENICE_MODELS_URL:
        logger.warning("VENICE_MODELS_URL is empty. Skipping model sync.")
        return
...
class AnimateDurationView(OwnerLockedView):
    ...
    async def _run(self, interaction: discord.Interaction, seconds: int):
        ...
        if not attachment:
            await interaction.followup.send("❌ No image attachment found on source message.", ephemeral=True)
            return

        try:
            image_bytes = await attachment.read()
        except Exception:
            await interaction.followup.send("❌ Could not read source image bytes.", ephemeral=True)
            return

        if not _looks_like_image(image_bytes):
            await interaction.followup.send("❌ Source attachment is not a valid image.", ephemeral=True)
            return
    ...
    async def sec5(self, interaction: discord.Interaction, _: discord.ui.Button):
        await self._run(interaction, 5)

    @discord.ui.button(label="10 seconds", style=discord.ButtonStyle.primary)
    async def sec10(self, interaction: discord.Interaction, _: discord.ui.Button):
        await self._run(interaction, 10)

    @discord.ui.button(label="15 seconds", style=discord.ButtonStyle.danger)
    async def sec15(self, interaction: discord.Interaction, _: discord.ui.Button):
        await self._run(interaction, 15)
