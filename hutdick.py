...
class HutDickCog(commands.Cog):
    ...
    async def hutdick(
            ...
            async for msg in channel.history(limit=search_limit):
                ...
                for reaction in msg.reactions:
                    ...
                    if emoji_key == HUT_REACTION:
                        total_points += max(reaction.count - 1, 0)
        ...
        if top_user_ids:
            await interaction.followup.send("Top 3 users: " + " ".join(f"<@{uid}>" for uid in top_user_ids), ephemeral=not post)
