...
class MoreInfoButton(discord.ui.Button):
    def __init__(self, full_text):
        super().__init__(label="[more info]", style=discord.ButtonStyle.secondary)
...
class AspectRatioView(discord.ui.View):
    def __init__(self, session, variant, prompt_text, hidden_suffix, author):
        ...
        if len(self.full_info_text) > 100:
            self.truncated_info = self.full_info_text[:100] + "... [more info]"
            self.add_item(MoreInfoButton(self.full_info_text))
        else:
            self.truncated_info = self.full_info_text
    ...
    async def generate_image(self, interaction: discord.Interaction, width: int, height: int):
        ...
        content = (
            f"🎨 Generated by: {self.author.mention}\n"
