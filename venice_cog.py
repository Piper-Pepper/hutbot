...
class AspectRatioView(discord.ui.View):
    ...
    async def generate_image(self, interaction: discord.Interaction, width: int, height: int):
        ...
        for i in range(1, 6):
            ...
            try:
                await progress_msg.edit(content=f"⏳ Generating image... {i*20}%")
            except:
                pass
        ...
        # Compose info string (negative prompt, cfg, steps, nsfw/sfw)
        info_text = (
        ...
        # Truncate after 100 chars with [more info]
        if len(info_text) > 100:
            truncated = info_text[:100] + "... [more info]"
            info_display = f"{truncated}\n||{info_text}||"
        else:
        ...
        await interaction.followup.send(content=content, file=file, ephemeral=False)

        # Button-Überprüfung nach dem Post
