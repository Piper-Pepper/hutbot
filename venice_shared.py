...
- Quota stores ONLY via get_quota_store(). TIER_RULES is the single source of truth.
- Video models: VIDEO_MODEL_PROFILES (image-to-video) and
  TEXT_VIDEO_MODEL_PROFILES (text-to-video). One entry = one button.
- Video files NEVER live fully in memory. Use the disk helpers.
- Every video feature is gated behind has_video_access().
"""
...
def setup_logging(force: bool = False) -> None:
    """Call ONCE from your entrypoint BEFORE loading cogs."""
    ...
    root.info("Logging initialised | level=%s | file=%s", LOG_LEVEL, LOG_FILE or "-")


def log_memory_usage(tag: str = "") -> Optional[int]:
    """Current RSS in MB, or None if /proc is unavailable."""
    try:
...
class RollingQuotaStore:
    """24h window anchored on first use, persisted as JSON. I/O in a thread."""
    ...
    def _state(self, entry: dict[str, int], limit: int, now_ts: int) -> dict[str, int]:
        ...
        if start > 0:
            ...
            reset_at, reset_in = 0, self.window_seconds
        return {
            "used": used, "limit": limit, "remaining": max(0, limit - used),
            "start": start, "reset_in": reset_in, "reset_at": reset_at,
        }
    ...
    async def reserve(
        ...
        now_ts = int(time.time())
        limit, amount = max(0, int(limit)), max(0, int(amount))
        async with self.lock:
            db = await self._read()
            ...
            token = {"guild_id": guild_id, "user_id": user_id,
                     "amount": amount, "start": entry["start"]}
            logger.debug("Quota +%s: g=%s u=%s used=%s/%s",
                         amount, guild_id, user_id, entry["used"], limit)
    ...
    async def rollback(self, token: Optional[dict[str, int]]) -> None:
        ...
        async with self.lock:
            ...
            if entry["start"] == token_start and entry["used"] > 0:
                ...
                db[key] = entry
                await self._write(db)
                logger.debug("Quota -%s: g=%s u=%s", amount, guild_id, user_id)
    ...
    async def prune(self) -> int:
        ...
        async with self.lock:
            db = await self._read()
            before = len(db)
            alive = {k: v for k, v in db.items()
                     if self._normalize(v, now_ts)["start"] > 0}
            if len(alive) != before:
...
def get_quota_store(
    ...
    if store is None:
        ...
        logger.warning("Quota store %s already registered with window=%ss.",
                       resolved, store.window_seconds)
    return store
...
# Shown in lock messages. ANY tier role grants access, see has_video_access().
VIDEO_MIN_ROLE_ID = env_int("VIDEO_MIN_ROLE_ID", 1377051179615522926)
...
def has_video_access(member: Optional[discord.Member]) -> bool:
    """
    Video gate. Tier 1 is the minimum, but ANY tier role qualifies - a member
    holding only the Tier-3 role must not be locked out just because the
    Tier-1 role was never assigned. Mirrors get_video_budget_for_member().
    """
    return get_member_tier(member) >= 1
...
def closest_aspect_ratio(aspect: str, allowed: Optional[list[str]]) -> str:
    """Numeric (W/H) match. Tokens like 'auto'/'adaptive' are skipped."""
    if not allowed:
        return aspect
    ...
    if target is None:
        return numeric[0]
    scored = [(a, v) for a, v in ((x, _val(x)) for x in numeric) if v is not None]
    return min(scored, key=lambda p: abs(p[1] - target))[0] if scored else numeric[0]
...
def file_head(path: str | Path, n: int = 16) -> bytes:
    """Read the first n bytes of a file without loading it into RAM."""
    try:
...
def file_looks_like_video(path: Optional[str | Path]) -> bool:
    return looks_like_video(file_head(path)) if path else False


def file_looks_like_image(path: Optional[str | Path]) -> bool:
    return looks_like_image(file_head(path)) if path else False
...
def infer_image_mime(binary: bytes) -> str:
    return {"png": "image/png", "jpg": "image/jpeg",
        ...
        infer_image_ext(binary), "image/png")
...
def prepare_source_image_for_upload(
    ...
    """
    Shrink a source image before it is base64-inlined into a queue payload.
    Base64 inflates by ~33% and the JSON encoder copies it again.

    min_short_side prevents downscaling below a model's hard minimum
    (WAN 3.0 = 240px). Returns the input unchanged if Pillow is missing.
    """
    ...
    if dims and min_short_side > 0:
        short, long_side = min(dims), max(dims)
        if short > 0:
            max_side = max(max_side, int(long_side * (min_short_side / short)))
    ...
    try:
        ...
        for q in (92, 86, 80, 74, 68, 60, 52, 45):
            ...
            if len(data) <= max_bytes:
                logger.info("Source image shrunk: %s -> %s (q=%s)",
                            human_bytes(len(binary)), human_bytes(len(data)), q)
                return data
        logger.warning("Source image still %s after compression.", human_bytes(len(data)))
        return data
    except Exception as e:
...
def extract_image_from_json_obj(obj: Any, depth: int = 0) -> Optional[bytes]:
    ...
    if isinstance(obj, dict):
        for key in ("image", "image_base64", "imageBase64", "b64_json",
...
def extract_urls_from_payload(payload: Any) -> list[str]:
    ...
    if not isinstance(payload, (dict, list)):
        return urls
    interesting = {"download_url", "url", "result_url",
                   "video_url", "file_url", "asset_url"}
...
def cleanup_temp_files(*paths: Optional[str | Path]) -> None:
    """Delete temp files, never raising. Safe in a finally block."""
    for p in paths:
        if not p:
            continue
        try:
...
async def compress_video_file(
    ...
    """
    Shrink a video below target_bytes with ffmpeg (H.264 + AAC), disk only.

    Computes the bitrate that fits the byte budget, then walks down a
    resolution ladder until the encoder lands under target. Audio is kept
    as long as possible.

    Returns (output_path, note). None on failure. If the source already fits,
    the source path is returned unchanged. Caller owns cleanup.
    """
    src = Path(src_path)
    if not src.exists():
    ...
    if not ffmpeg_available():
        logger.error("ffmpeg not found (FFMPEG_BIN=%s).", FFMPEG_BIN)
    ...
    logger.info("Compressing: %s -> %s | dur=%.1fs budget=%skbit/s",
                human_bytes(original), human_bytes(target_bytes), duration, total_kbit)

    if total_kbit <= VIDEO_MIN_VIDEO_KBIT:
        return None, (f"clip too long for a {human_bytes(target_bytes)} limit "
    ...
    for idx, (height, video_kbit, keep_audio) in enumerate(attempts):
        ...
        if code != 0 or not out_path.exists():
            ...
            logger.warning("ffmpeg pass %s failed (code=%s, %.1fs): %s",
                           idx + 1, code, took, msg)
        ...
        logger.info("ffmpeg pass %s done in %.1fs: %s at %sp%s",
                    idx + 1, took, human_bytes(size), height,
                    "" if keep_audio else " (muted)")

        if size <= target_bytes:
            note = (f"{human_bytes(original)} → {human_bytes(size)} @ {height}p"
...
def guild_upload_limit_bytes(guild: Optional[discord.Guild]) -> int:
    """Upload cap for a guild without an Interaction (10MB unboosted)."""
    if DISCORD_UPLOAD_LIMIT_FORCE_MB > 0:
        return DISCORD_UPLOAD_LIMIT_FORCE_MB * 1024 * 1024
...
def channel_upload_limit_bytes(channel: Optional[discord.abc.Messageable]) -> int:
    return guild_upload_limit_bytes(getattr(channel, "guild", None))
    ...
    for a in msg.attachments:
        ...
        if ctype.startswith("image/") or name.endswith(
            ...
            if isinstance(a.url, str) and a.url.startswith("http"):
                return a.url
    for emb in msg.embeds:
        u = (emb.image.url if emb.image else None) or (
...
async def track_ephemeral_message(
    interaction: discord.Interaction, msg: Optional[discord.Message]
):
    if msg:
...
async def send_ephemeral(
    ...
    **kwargs,
    ...
    - views with persistent_ephemeral = True -> NOT tracked
    - content starting with ❌ ⛔ 🔒 🚫 ⚠️ 🛑 -> NOT tracked
    - everything else -> tracked, deleted on the next cleanup pass
    """
    payload = dict(kwargs)
...
def build_image_quota_text(member: Optional[discord.Member], state: dict[str, int]) -> str:
    ...
    if nxt:
        nt, cfg = nxt
        lines.append(f"🚀 Next unlock: **Tier {nt}** (<@&{cfg['role_id']}>, "
                     f"Level {cfg['level']}) → **{cfg['image_limit']} images/24h**.")
    else:
...
def build_video_quota_text(
    tier: int, state: dict[str, int], min_duration: Optional[int] = None
) -> str:
    lines = [
        "⛔ Not enough video seconds left in your 24h window.",
        f"Used: **{state['used']}/{state['limit']}s** • "
        f"Remaining: **{state['remaining']}s**",
    ]
    if min_duration:
        lines.append(f"This model needs at least **{min_duration}s** per render.")
    lines.append(f"⏳ {format_reset_line(state)}")
    lines.append(f"Current tier: **T{tier}**")
    nxt = next_tier(tier)
    if nxt:
        nt, cfg = nxt
        lines.append(f"🚀 Next unlock: **Tier {nt}** (<@&{cfg['role_id']}>, "
                     f"Level {cfg['level']}) → **{cfg['video_budget_sec']}s/day**.")
    lines.append(f"Tier budgets: `{video_tier_line()}`")
    return "\n".join(lines)
...
async def send_tier_locked_message(
    interaction: discord.Interaction, min_tier: int, feature: str = "This feature"
):
...
async def send_role_locked_message(
    interaction: discord.Interaction, required_role_id: int, feature: str = "This feature"
):
    match = tier_for_role_id(required_role_id)
    if match:
        tier, cfg = match
        text = (f"🔒 You need **Tier {tier}**, **Level {cfg['level']}** and the Role "
...
async def send_resolution_lock_message(
    interaction: discord.Interaction, resolution: str,
    required_tier: int, current_tier: int,
):
...
def build_progress_embed(
    *,
    ...
    emb = discord.Embed(title=title, description=user.mention,
                        color=color, timestamp=utc_now())
    emb.add_field(name="Prompt",
                  value=f"```{codeblock_safe(trim(prompt, 420))}```", inline=False)
    emb.add_field(name="Progress",
                  value=f"`{progress_bar(percent)} {percent}%`", inline=False)
    emb.add_field(name="Status",
                  value="\n".join(f"• {ln}" for ln in status_lines if ln), inline=False)
    emb.add_field(
        name=quota_name,
        value=(f"• Used: `{used}/{limit}{quota_unit}`\n"
...
async def run_with_progress(
    ...
    """Drive a progress bar while `task` runs. Returns elapsed seconds."""
    started = time.monotonic()
    last_percent = -1
...
def register_starter_reposter(
    channel_id: int, fn: Callable[[discord.TextChannel], Awaitable[None]]
) -> None:
...
# Aspect ratios accepted as SOURCE hints from the image cogs.
...
VENICE_VIDEO_T2V_MODEL_WAN3 = env_str(
    "VENICE_VIDEO_T2V_MODEL_WAN3", "wan-3-0-text-to-video")
VENICE_VIDEO_T2V_MODEL_LTX25 = env_str(
    "VENICE_VIDEO_T2V_MODEL_LTX25", "ltx-2-5-pro-text-to-video")
VENICE_VIDEO_T2V_MODEL_MINIMAX = env_str(
    "VENICE_VIDEO_T2V_MODEL_MINIMAX", "minimax-h3-max-text-to-video")
...
TEXT_VIDEO_CHANNEL_ID = env_int("TEXT_VIDEO_CHANNEL_ID", 1545968684466446346)

# Buttons show the bare model name. Uncensored models carry 🔞.
#
# button_label / button_style -> UI
# resolution                  -> exact string the API expects (MiniMax: "768P")
# durations                   -> allowed seconds; the picker shows only these
# require_aspect_ratio        -> send aspect_ratio in the payload at all
# aspect_ratio_auto           -> "match the source" token
# allowed_aspect_ratios       -> fallback when no auto token exists
# prompt_limit                -> provider-side character cap
# min_short_side              -> minimum short edge of the source image
# est_seconds_per_second      -> wall-clock cost per output second (timeouts)
...
# Text-to-video. No source image, so the aspect ratio is user-picked.
# NOTE: MiniMax has no 720p tier (only 480P / 768P) - 768P is the closest.
TEXT_VIDEO_MODEL_PROFILES: dict[str, dict[str, Any]] = {
    VENICE_VIDEO_T2V_MODEL_WAN3: {
        "button_label": "Wan 3.0 🔞",
        "button_style": discord.ButtonStyle.danger,
        "resolution": "720p",
        "durations": [5, 10, 15, 20, 25],
    ...
    VENICE_VIDEO_T2V_MODEL_MINIMAX: {
        "button_label": "MiniMax H3 Max 🔞",
        "button_style": discord.ButtonStyle.primary,
        "resolution": "768P",
        "durations": [5, 8, 10, 12, 15],
...
_T2V_PROFILE_FALLBACK: dict[str, Any] = {
    "button_label": "Video",
    "button_style": discord.ButtonStyle.primary,
    "resolution": "720p",
    "durations": [5],
...
def get_t2v_profile(model_id: str) -> dict[str, Any]:
    return TEXT_VIDEO_MODEL_PROFILES.get((model_id or "").strip(), _T2V_PROFILE_FALLBACK)


def any_video_profile(model_id: str) -> dict[str, Any]:
    """Look a model up in BOTH catalogs. Used by the render cog."""
    key = (model_id or "").strip()
    if key in VIDEO_MODEL_PROFILES:
        return VIDEO_MODEL_PROFILES[key]
    if key in TEXT_VIDEO_MODEL_PROFILES:
        return TEXT_VIDEO_MODEL_PROFILES[key]
    return _VIDEO_PROFILE_FALLBACK
...
def is_known_t2v_model(model_id: str) -> bool:
    return (model_id or "").strip() in TEXT_VIDEO_MODEL_PROFILES


def is_any_known_video_model(model_id: str) -> bool:
    return is_known_video_model(model_id) or is_known_t2v_model(model_id)
...
def get_model_durations(model_id: str) -> list[int]:
    return list(any_video_profile(model_id)["durations"])


def get_model_resolution(model_id: str) -> Optional[str]:
    return any_video_profile(model_id).get("resolution")


def get_model_prompt_limit(model_id: str) -> int:
    return int(any_video_profile(model_id).get("prompt_limit") or 3000)


def get_model_min_short_side(model_id: str) -> int:
    return int(any_video_profile(model_id).get("min_short_side") or 0)


def get_model_speed_factor(model_id: str) -> int:
    return int(any_video_profile(model_id).get("est_seconds_per_second") or 40)


def get_model_label(model_id: str) -> str:
    """Display label for a VIDEO model (both catalogs)."""
    return str(any_video_profile(model_id).get("button_label") or model_id)


def estimate_render_seconds(model_id: str, output_seconds: int) -> int:
    """
    Rough wall-clock estimate for a render. Drives poll timeouts and the
    ETA shown on the progress bar.
    """
...
def resolve_video_aspect_ratio(model_id: str, source_aspect: str) -> Optional[str]:
    """
    aspect_ratio for the image-to-video queue payload.
    - no aspect support (MiniMax I2V) -> None, field omitted
    - auto token -> 'adaptive' (WAN) / 'auto' (LTX)
    - otherwise -> closest numeric match
    """
...
def resolve_t2v_aspect_ratio(model_id: str, chosen: str) -> str:
    profile = get_t2v_profile(model_id)
    allowed = list(profile.get("aspect_ratios") or ["16:9"])
    if chosen in allowed:
        return chosen
    return closest_aspect_ratio(chosen or str(profile["default_aspect"]), allowed)


def check_source_image_for_model(
    model_id: str, image_bytes: Optional[bytes]
) -> Optional[str]:
    """Error string if the source image violates a model constraint, else None."""
    ...
    if min(dims) < min_side:
        return (f"Source image too small: short edge is {min(dims)}px, "
                f"this model requires at least {min_side}px.")
    return None


# =================================================
# ANIMATE UI (image-to-video)
# =================================================
class AnimatePromptModal(discord.ui.Modal):
    def __init__(
        self, owner_id: int, source_channel_id: int, source_message_id: int,
        base_prompt: str, ratio: str, model_id: str,
    ):
        self.owner_id = owner_id
        ...
        super().__init__(title=f"🎬 {get_video_profile(model_id)['button_label']}"[:45])

        # Discord modals cap at 4000 chars regardless of provider limits.
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not is_known_video_model(self.model_id):
            ...
            await send_ephemeral(
                interaction,
                "❌ This button points at a retired model. Generate a new image.")
            return
        ...
        try:
            ...
            await send_ephemeral(interaction, "❌ Could not load your video quota.")
            return
        ...
        if not allowed:
            await send_ephemeral(
                ...
                build_video_quota_text(
                    int(info["tier"]), info,
                    min(model_durations) if model_durations else None),
            )
            return
...
class AnimateDurationView(OwnerLockedView):
    def __init__(
        self, owner_id: int, source_channel_id: int, source_message_id: int,
        prompt_text: str, ratio: str, allowed_durations: list[int], model_id: str,
    ):
        ...
        if not self.allowed_durations:
            self.add_item(discord.ui.Button(
                label="No duration available", disabled=True,
                style=discord.ButtonStyle.secondary))
            return

        for idx, sec in enumerate(self.allowed_durations):
            style = (discord.ButtonStyle.success if sec <= 8
                     else discord.ButtonStyle.danger if sec >= 20
                     else discord.ButtonStyle.primary)
            b = discord.ui.Button(label=f"{sec}s", style=style, row=idx // 5)
    ...
    async def _run(self, interaction: discord.Interaction, seconds: int):
        ...
        with contextlib.suppress(Exception):
            ...
            if int(rem.get("remaining", 0)) < seconds:
                await send_ephemeral(
                    interaction, build_video_quota_text(int(rem.get("tier", 0)), rem))
                return
        ...
        shared_session = (getattr(face_cog, "session", None)
        ...
        prompt = trim(
            (self.prompt_text or "").strip() or "Animate this image with natural motion.",
            get_model_prompt_limit(self.model_id),
        )

        logger.info("Animate | user=%s model=%s dur=%ss ar=%s src=%s",
...
class AnimateEphemeralView(OwnerLockedView):
    """
    Attached to a successful image/face-image post. Buttons are built from
    VIDEO_MODEL_PROFILES, so a new variant is a one-line table edit.

    persistent_ephemeral = True keeps send_ephemeral from tracking this
    message, so cleanup_user_ephemerals() will NOT delete it after a render
    and the buttons stay clickable for the full 30 minute timeout.
    """
    persistent_ephemeral = True

    def __init__(
        self, owner_id: int, source_channel_id: int,
        source_message_id: int, prompt_text: str, ratio: str,
    ):
...
def is_t2v_starter_message(msg: discord.Message) -> bool:
    return T2V_STARTER_MARKER in (msg.content or "")


class T2VPromptModal(discord.ui.Modal):
    def __init__(self, owner_id: int, channel_id: int, model_id: str):
        self.owner_id = owner_id
        self.channel_id = channel_id
        self.model_id = model_id
        profile = get_t2v_profile(model_id)
        super().__init__(title=f"🎬 {profile['button_label']}"[:45])

        self.prompt = discord.ui.TextInput(
            label="Video prompt",
            style=discord.TextStyle.paragraph,
            required=True,
            max_length=min(4000, int(profile["prompt_limit"])),
            placeholder="Scene, subject, motion, camera, lighting...",
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not has_video_access(interaction.user):
            await send_video_role_locked(interaction)
            return
        if not is_known_t2v_model(self.model_id):
        ...
        if not allowed:
            await send_ephemeral(
                interaction,
                build_video_quota_text(
                    int(info["tier"]), info, min(profile["durations"])),
            )
            return
...
class T2VOptionsView(OwnerLockedView):
    def __init__(
        self, owner_id: int, channel_id: int, model_id: str,
        prompt_text: str, allowed_durations: list[int],
    ):
        super().__init__(owner_id=owner_id, timeout=300)
        self.channel_id = channel_id
        self.model_id = model_id
        ...
        profile = get_t2v_profile(model_id)
        self.aspect = str(profile["default_aspect"])
        ratios = list(profile["aspect_ratios"])[:25]

        select = discord.ui.Select(
            placeholder=f"Aspect ratio • {self.aspect}",
            options=[discord.SelectOption(label=r, value=r,
                                          default=(r == self.aspect)) for r in ratios],
            row=0,
        )
        select.callback = self._on_ratio
        self.add_item(select)
        self._select = select

        for idx, sec in enumerate(sorted(set(allowed_durations))):
            style = (discord.ButtonStyle.success if sec <= 8
                     else discord.ButtonStyle.danger if sec >= 20
                     else discord.ButtonStyle.primary)
            b = discord.ui.Button(label=f"{sec}s", style=style, row=1 + idx // 5)
            b.callback = self._make_run(sec)
            self.add_item(b)

    async def _on_ratio(self, interaction: discord.Interaction):
        self.aspect = self._select.values[0]
        self._select.placeholder = f"Aspect ratio • {self.aspect}"
        for opt in self._select.options:
            opt.default = (opt.value == self.aspect)
        with contextlib.suppress(Exception):
            await interaction.response.edit_message(view=self)

    def _make_run(self, seconds: int):
        async def _cb(interaction: discord.Interaction):
            await self._run(interaction, seconds)
        return _cb
    ...
    async def _run(self, interaction: discord.Interaction, seconds: int):
        ...
        if channel is None:
            await send_ephemeral(interaction, "❌ Target channel not usable.")
            return

        prompt = trim(self.prompt_text.strip(), get_model_prompt_limit(self.model_id))
        aspect = resolve_t2v_aspect_ratio(self.model_id, self.aspect)

        logger.info("T2V | user=%s model=%s dur=%ss ar=%s",
                    interaction.user.id, self.model_id, seconds, aspect)

        await video_cog.text_to_video(
            interaction=interaction,
            prompt=prompt,
            aspect=aspect,
            seconds=seconds,
...
class T2VStarterView(discord.ui.View):
    """Persistent starter view in the text-to-video channel."""

    def __init__(self, channel_id: int = TEXT_VIDEO_CHANNEL_ID):
        super().__init__(timeout=None)
        self.channel_id = int(channel_id)

        for idx, (model_id, profile) in enumerate(TEXT_VIDEO_MODEL_PROFILES.items()):
            btn = discord.ui.Button(
                label=profile["button_label"],
                style=profile["button_style"],
                row=idx // 5,
                custom_id=f"t2v:{model_id}",
            )
            btn.callback = self._make_callback(model_id)
            self.add_item(btn)
