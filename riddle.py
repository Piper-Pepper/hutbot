...
# XP reminder channel (requested)
XP_NOTIFY_CHANNEL_ID = int(os.getenv("RIDDLE_XP_NOTIFY_CHANNEL_ID", "1381754826710585527"))
...
XP_COPY_BUTTON_ID = "riddle_xp_copy"
XP_DONE_BUTTON_ID = "riddle_xp_done"
...
def build_xpadd_commands(member_mention: str, member_name: str, xp_amount: int) -> tuple[str, str]:
    ...
    # name-based command requested + mention-safe alternative
...
class RiddleUpsertModal(Modal):
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        try:
            rid = await self.cog._upsert_slot_riddle(
            ...
            await self.cog._activate_next_from_pool(interaction.guild.id)
            await self.cog._publish_active_riddle(
                guild_id=interaction.guild.id,
                force_repost=False,
            ...
            logger.exception("Slot upsert failed: %s", e)
...
class VoteSuccessButton(discord.ui.Button):
    ...
    async def callback(self, interaction: Interaction):
        ...
        # XP reminder message with copy workflow
        try:
            await self.cog._send_xp_reminder_notification(
                guild=interaction.guild,
                solver_id=submitter_id,
                solver_mention=submitter_mention,
                solver_name=submitter_name,
                xp_gain=to_int(ctx.get("xp_gain"), 0),
                award_text=ctx.get("award"),
                riddle_id=to_int(ctx.get("riddle_id"), 0),
                riddle_no=to_int(ctx.get("riddle_no"), 0),
                riddle_text=ctx.get("riddle_text") or ""
            )
        except Exception as e:
            logger.exception("Failed to send XP reminder notification: %s", e)

        # Activate next slot and post next riddle
        try:
            ...
            logger.exception("Failed to activate next slot after solve: %s", e)
...
# =========================
# UI - XP REMINDER WORKFLOW
# =========================
class CopyXPCommandModal(Modal):
    def __init__(self, command_text: str):
        super().__init__(title="Copy XP Command")
    ...
    async def on_submit(self, interaction: Interaction):
        cmd = clean_value(self.command.value) or ""
        await interaction.response.send_message(f"✅ Command ready:\n`{cmd}`", ephemeral=True)


class XPTaskCopyButton(discord.ui.Button):
    def __init__(self, cog: "RiddleSystemSQL"):
        super().__init__(
            label="📋 Copy /xpadd",
            style=discord.ButtonStyle.secondary,
            custom_id=XP_COPY_BUTTON_ID
        )
        self.cog = cog

    async def callback(self, interaction: Interaction):
        if interaction.message is None:
            await interaction.response.send_message("❌ XP task message not found.", ephemeral=True)
            return

        task = await self.cog._get_xp_task_by_notify_message(interaction.message.id)
        if not task:
            await interaction.response.send_message("❌ XP task is missing.", ephemeral=True)
            return

        command_text = task.get("command_text") or ""
        await interaction.response.send_modal(CopyXPCommandModal(command_text))


class XPTaskDoneButton(discord.ui.Button):
    def __init__(self, cog: "RiddleSystemSQL"):
        super().__init__(
            label="✅ Mark XP Added",
            style=discord.ButtonStyle.success,
    ...
    async def callback(self, interaction: Interaction):
        ...
        if not member_has_role(interaction.user, RIDDLE_MANAGER_ROLE_ID):
            await interaction.response.send_message(
                f"❌ Only <@&{RIDDLE_MANAGER_ROLE_ID}> can mark XP as added.",
                ephemeral=True
            )
            return
        ...
        status, task = await self.cog._mark_xp_task_done_by_notify_message(
            notify_message_id=interaction.message.id,
            done_by=interaction.user.id
        )
        if status == "not_found":
            await interaction.response.send_message("❌ XP task is missing.", ephemeral=True)
            return
        if status == "already_done":
            await interaction.response.send_message("ℹ️ XP is already marked as added.", ephemeral=True)
            return
        if status != "ok" or not task:
            await interaction.response.send_message("❌ Could not update XP task.", ephemeral=True)
            return
        ...
        if interaction.message.embeds:
            ...
            embed = discord.Embed(title="🧠 XP Action Required", color=discord.Color.orange())

        embed.color = discord.Color.green()
        done_stamp = discord.utils.format_dt(dt.datetime.now(dt.timezone.utc), style="F")
        embed.add_field(
            name="Status",
            value=f"✅ Marked as paid by {interaction.user.mention} on {done_stamp}",
            inline=False
        )
        embed.set_footer(text="XP workflow completed")

        await interaction.response.edit_message(embed=embed, view=XPTaskDoneView())


class XPTaskActionView(LoggedPersistentView):
    def __init__(self, cog: "RiddleSystemSQL"):
        super().__init__(timeout=None)
        self.add_item(XPTaskCopyButton(cog))
        self.add_item(XPTaskDoneButton(cog))


class XPTaskDoneView(View):
    def __init__(self):
        super().__init__(timeout=None)
        self.add_item(discord.ui.Button(label="📋 Copy /xpadd", style=discord.ButtonStyle.secondary, disabled=True))
        self.add_item(discord.ui.Button(label="✅ XP Added", style=discord.ButtonStyle.success, disabled=True))
...
class SlotEditButton(discord.ui.Button):
    def __init__(self, cog: "RiddleSystemSQL", slot_no: int, filled: bool):
        super().__init__(
    ...
    async def callback(self, interaction: Interaction):
        ...
        current = await self.cog._get_open_slot_riddle(interaction.guild.id, self.slot_no)
        modal = RiddleUpsertModal(
            cog=self.cog,
            current_data=current,
            mention_override_id=None,
            slot_no=self.slot_no
        )
...
class SlotRefreshButton(discord.ui.Button):
    ...
    async def callback(self, interaction: Interaction):
        ...
        slot_map = await self.cog._open_slot_map(interaction.guild.id)
        embed = self.cog._build_slots_overview_embed(interaction.guild, slot_map)
        view = SlotManagerView(cog=self.cog, owner_id=interaction.user.id, slot_map=slot_map)
        await interaction.response.edit_message(embed=embed, view=view)


class SlotManagerView(View):
    def __init__(self, *, cog: "RiddleSystemSQL", owner_id: int, slot_map: dict[int, dict]):
...
class RiddleSystemSQL(commands.Cog):
    ...
    async def cog_load(self):
        ...
        self.bot.add_view(XPTaskActionView(self))
    ...
    async def _init_db(self):
        ...
        CREATE TABLE IF NOT EXISTS xp_tasks (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            guild_id INTEGER NOT NULL,
            riddle_id INTEGER NOT NULL,
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS idx_xp_tasks_notify_message
        ON xp_tasks(notify_message_id) WHERE notify_message_id IS NOT NULL;
        """
        ...
        async with self.db_lock:
            ...
            if "slot_no" not in cols:
                await self.db.execute("ALTER TABLE riddles ADD COLUMN slot_no INTEGER")
            if "is_active" not in cols:
                await self.db.execute("ALTER TABLE riddles ADD COLUMN is_active INTEGER NOT NULL DEFAULT 0")

            # old single-open index (remove)
            await self.db.execute("DROP INDEX IF EXISTS idx_riddles_open_guild")
            ...
            # repair pool integrity before unique slot index
    ...
    async def _repair_pool_integrity_locked(self):
        """
        Runs with db_lock held.
        Repairs:
        - open riddles get valid slot_no 1..MAX (extra rows get closed)
        - only one active open per guild
        """
        assert self.db is not None
        now = now_iso_utc()
        ...
        for gid in guild_ids:
            ...
            need_assign: list[dict] = []

            # keep first valid occupant for each slot, assign others later
            for r in rows:
                rid = to_int(r["id"], 0)
                slot = to_int(r.get("slot_no"), 0)
                if 1 <= slot <= MAX_RIDDLE_SLOTS and slot not in used_slots:
                    used_slots[slot] = rid
                else:
                    need_assign.append(r)

            # assign free slots
            free_slots = [s for s in range(1, MAX_RIDDLE_SLOTS + 1) if s not in used_slots]
            for r in need_assign:
                rid = to_int(r["id"], 0)
                if free_slots:
                    ...
                    # overflow open rows => close gracefully
                    await self.db.execute(
                        """
                        UPDATE riddles
            ...
            await self.db.execute(
                "UPDATE riddles SET is_active=0 WHERE guild_id=? AND status='open'",
                (gid,)
            )
            cur = await self.db.execute(
                """
            ...
            if first:
                await self.db.execute(
                    "UPDATE riddles SET is_active=1 WHERE id=?",
                    (to_int(first["id"], 0),)
                )
    ...
    # -------------------------
    # slot pool operations
    # -------------------------
    async def _open_slot_map(self, guild_id: int) -> dict[int, dict]:
    ...
    async def _get_open_slot_riddle(self, guild_id: int, slot_no: int) -> Optional[dict]:
        return await self._fetchone(
            """
            SELECT *
            FROM riddles
            WHERE guild_id=? AND status='open' AND slot_no=?
            LIMIT 1
            """,
            (guild_id, slot_no)
        )

    async def _get_active_open_riddle(self, guild_id: int) -> Optional[dict]:
    ...
    async def _activate_next_from_pool(self, guild_id: int) -> Optional[dict]:
        if self.db is None:
            return None
        ...
        async with self.db_lock:
            ...
            try:
                ...
                await self.db.execute(
                    "UPDATE riddles SET is_active=1, updated_at=? WHERE id=?",
                    (now_iso_utc(), nxt_id)
                )
                await self.db.commit()
            ...
            try:
                ...
                if row:
                    ...
                    cur = await self.db.execute(
                        ...
                        VALUES (?, ?, ?, 0, ?, ?, ?, ?, ?, ?, 'open', ?, ?, ?)
                        """,
                        (guild_id, next_no, slot_no, text, solution, award, image_url, solution_url, button_role_id, user_id, now, now)
                    )
    ...
    async def _close_active_riddle(self, guild_id: int, closed_by: int) -> Optional[dict]:
        if self.db is None:
            return None
        ...
        async with self.db_lock:
            ...
            try:
                cur = await self.db.execute(
                    ...
                    ORDER BY is_active DESC, slot_no ASC, id ASC
                    LIMIT 1
                    """,
                    (guild_id,)
                )
    ...
    # -------------------------
    # active riddle access
    # -------------------------
    async def _get_open_riddle(self, guild_id: int) -> Optional[dict]:
        return await self._get_active_open_riddle(guild_id)
    ...
    async def _clear_other_open_post_refs(self, guild_id: int, active_id: int):
        await self._execute(
            """
            UPDATE riddles
    ...
    async def _create_xp_task(
        ...
        *,
        ...
        xp_amount: int,
        command_text: str,
        command_alt_text: Optional[str],
        notify_channel_id: int
    ) -> Optional[int]:
        _, lid = await self._execute(
            """
            INSERT INTO xp_tasks (
                guild_id, riddle_id, solver_id, xp_amount, command_text, command_alt_text,
                status, created_at, notify_channel_id
            )
            VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?)
            """,
            (guild_id, riddle_id, solver_id, max(0, xp_amount), command_text, command_alt_text, now_iso_utc(), notify_channel_id)
        )
        return lid if lid > 0 else None

    async def _set_xp_task_notify_message(self, task_id: int, message_id: int) -> bool:
        rc, _ = await self._execute(
            "UPDATE xp_tasks SET notify_message_id=? WHERE id=?",
            (message_id, task_id)
        )
        return rc > 0

    async def _delete_xp_task(self, task_id: int):
        await self._execute("DELETE FROM xp_tasks WHERE id=?", (task_id,))

    async def _get_xp_task_by_notify_message(self, notify_message_id: int) -> Optional[dict]:
        return await self._fetchone(
            "SELECT * FROM xp_tasks WHERE notify_message_id=? LIMIT 1",
            (notify_message_id,)
        )

    async def _mark_xp_task_done_by_notify_message(self, *, notify_message_id: int, done_by: int) -> tuple[str, Optional[dict]]:
        if self.db is None:
            return "error", None
    ...
    async def _send_xp_reminder_notification(
        self,
        *,
        ...
        solver_name: str,
        xp_gain: int,
        award_text: Optional[str],
        riddle_id: int,
        riddle_no: int,
        riddle_text: str
    ):
        channel = await self._resolve_channel(XP_NOTIFY_CHANNEL_ID)
        if channel is None or not hasattr(channel, "send"):
            logger.warning("XP notify channel not found: %s", XP_NOTIFY_CHANNEL_ID)
            return

        guild_id = guild.id if guild else 0
        name_cmd, mention_cmd = build_xpadd_commands(solver_mention, solver_name, xp_gain)

        task_id = await self._create_xp_task(
            guild_id=guild_id,
            riddle_id=riddle_id,
            solver_id=solver_id,
            xp_amount=max(0, xp_gain),
            command_text=name_cmd,
            command_alt_text=mention_cmd,
            notify_channel_id=XP_NOTIFY_CHANNEL_ID
        )
        if not task_id:
            logger.warning("Failed to create XP task row.")
            return

        rn = riddle_no if riddle_no > 0 else riddle_id
        embed = discord.Embed(
            title="🧠 XP Action Required",
            description=f"{solver_mention} solved riddle #{rn}. Please add the XP manually.",
            color=discord.Color.orange(),
            timestamp=dt.datetime.now(dt.timezone.utc)
        )
        embed.add_field(name="Winner", value=f"{solver_mention}\n`{solver_name}`", inline=False)
        embed.add_field(name="XP to Add", value=f"`{max(0, xp_gain)}`", inline=True)
        embed.add_field(name="Award Input", value=award_text or "*None*", inline=True)
        embed.add_field(name="Copy Command (name-based)", value=f"`{name_cmd}`", inline=False)
        embed.add_field(name="Alternative (mention-safe)", value=f"`{mention_cmd}`", inline=False)
        embed.add_field(name="Riddle", value=truncate_text(riddle_text, 350) or "*No text*", inline=False)
        embed.set_footer(text="Click the button to open a copy field.")

        try:
            msg = await channel.send(
                content=f"<@&{RIDDLE_MANAGER_ROLE_ID}>",
                embed=embed,
                view=XPTaskActionView(self),
                allowed_mentions=discord.AllowedMentions(roles=True, users=False, everyone=False)
            )
        except Exception:
            await self._delete_xp_task(task_id)
            raise

        ok = await self._set_xp_task_notify_message(task_id, msg.id)
        if not ok:
            try:
                await msg.delete()
            except Exception:
    ...
    def _build_riddle_embed(self, guild: Optional[discord.Guild], riddle: dict) -> discord.Embed:
    ...
    async def _publish_active_riddle(
        self,
        *,
        guild_id: int,
        force_repost: bool,
        ...
        embed = self._build_riddle_embed(guild, active)
        ...
        existing_msg = await self._fetch_message_safe(active.get("posted_channel_id"), active.get("posted_message_id"))
        ...
        try:
            ...
            if existing_msg:
                await existing_msg.edit(
                    ...
                    allowed_mentions=discord.AllowedMentions(
                        roles=allow_role_ping,
                        users=False,
                        everyone=False
                    )
                )
                await self._clear_other_open_post_refs(guild_id, to_int(active["id"], 0))
                return "updated"
            ...
            await self._set_riddle_posted_message(to_int(active["id"], 0), msg.channel.id, msg.id)
            await self._clear_other_open_post_refs(guild_id, to_int(active["id"], 0))
            return "posted"
        except Exception as e:
            logger.exception("publish active riddle failed: %s", e)
            return "error"

    async def _startup_rebuild_messages(self):
        logger.info("Startup rebuild: deleting old submit/vote button posts...")

        # delete stale riddle submit posts
        await self._delete_button_messages_in_channel(
            RIDDLE_CHANNEL_ID,
            {SUBMIT_BUTTON_ID},
            limit=500
        ...
        # delete stale vote approval posts
        await self._delete_button_messages_in_channel(
            VOTE_CHANNEL_ID,
        ...
        # reset post refs for open riddles + pending vote message refs
        await self._execute(
            "UPDATE riddles SET posted_channel_id=NULL, posted_message_id=NULL, updated_at=? WHERE status='open'",
            (now_iso_utc(),)
        )
        await self._execute(
            "UPDATE submissions SET vote_message_id=NULL WHERE status='pending'"
        )

        # activate one per guild and repost fresh (without role ping spam on restart)
        ...
        for row in guild_rows:
            ...
            if gid <= 0:
                continue
            await self._activate_next_from_pool(gid)
            await self._publish_active_riddle(
                guild_id=gid,
                force_repost=True,
                extra_ping_role_id=None,
    ...
    # slot overview embed
    # -------------------------
    def _build_slots_overview_embed(self, guild: discord.Guild, slot_map: dict[int, dict]) -> discord.Embed:
        active_slot = None
        ...
        embed = discord.Embed(
            ...
            description="Manage up to 5 slots. The active slot is posted in the riddle channel.",
            color=discord.Color.blurple()
        )
    ...
    @app_commands.describe(
        slot="Pool slot number (1-5)",
        mention="Optional role to store as mention/button role"
    )
    ...
    async def riddle(
        ...
        current = await self._get_open_slot_riddle(interaction.guild.id, int(slot))
        modal = RiddleUpsertModal(
            cog=self,
            current_data=current,
    ...
    async def riddle_slots(self, interaction: Interaction):
        ...
        slot_map = await self._open_slot_map(interaction.guild.id)
        embed = self._build_slots_overview_embed(interaction.guild, slot_map)
        view = SlotManagerView(cog=self, owner_id=interaction.user.id, slot_map=slot_map)

        await interaction.followup.send(embed=embed, view=view, ephemeral=True)

    @app_commands.command(name="riddle_post", description="Force repost the currently active riddle.")
    ...
    async def riddle_post(self, interaction: Interaction, ping_role: Optional[Role] = None):
        ...
        await self._activate_next_from_pool(interaction.guild.id)
        result = await self._publish_active_riddle(
            guild_id=interaction.guild.id,
            force_repost=True,
        ...
        if result in ("posted", "updated"):
            await interaction.followup.send("✅ Active riddle was reposted.", ephemeral=True)
            return

        await interaction.followup.send("❌ Posting failed.", ephemeral=True)

    @app_commands.command(name="riddle_view", description="Private preview of the active riddle + solved preview.")
    @app_commands.guild_only()
    @riddle_manager_required()
    async def riddle_view(self, interaction: Interaction):
        ...
        if 1 <= slot_no <= MAX_RIDDLE_SLOTS:
            riddle_embed.add_field(name="📦 Pool Slot", value=str(slot_no), inline=True)
        if mention_group:
    ...
    @app_commands.command(name="riddle_close", description="Close the active riddle as unsolved and move to next slot.")
    @app_commands.guild_only()
    @riddle_manager_required()
    async def riddle_close(self, interaction: Interaction):
        ...
        riddle = await self._close_active_riddle(interaction.guild.id, interaction.user.id)
        if not riddle:
        ...
        await interaction.followup.send("✅ Active riddle closed. Next slot posted (if available).", ephemeral=True)
